...
if ($dbSingleUse->HasRecords($query)) { 
	...
	foreach ($searchArray as $j => $searchRow) {
		...
		//$filterClearingGroup;
			echo '<tr>';
			echo '<td class="lotLinkCellInTable" ><a href="index.php?myAction=Lot&lotNumber='.$searchRow["lotNumber"].'&siteShortName='.$searchRow["siteShortName"].'">'.$searchRow["lotNumber"];
			if ($siteShortName <= "") {
