...
if ($siteShortName > "") {
	$query = $query.'  where moveInDate < curdate()';
}
else {
...
if ($db2->Query($query)) { 
	...
	while ($resultRow = $db2->Row() ) {
		...
		if ($showThisRow == true)  {
			echo '<tr>';
			echo '<td class="lotLinkCellInTable" ><a href="index.php?myAction=Lot&lotNumber='.$resultRow->lotNumber.'&siteShortName='.$resultRow->siteShortName.'">'.$resultRow->lotNumber;
			if ($siteShortName <= "") {
