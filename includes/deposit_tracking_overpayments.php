...
<h2>Deposits With Extra Payments</h2>
<small>(15 most recent deposits with extra payments applied)</small>
<?php

require_once ("classes/misc_functions.php");

function depositsOverpaymentsHeader() {
	echo '  <tr>';
...
and dueDate < CURDATE()
order by a.dueDate limit 15";
$rowNum = 0;
if ($db->Query($query)) { 
	while ($depositRow = $db->Row() ) {
		if ($rowNum == 0) {
			depositsOverpaymentsHeader();
		}
		$rowNum = $rowNum + 1;
