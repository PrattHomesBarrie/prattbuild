...
<big><b>Closings Summary (incl. unsigned)</b></big><small> (includes calculation on amended closing date) </small>
<?php

function siteSummaryHeader($startYear,$endYear) {
echo '
...
if ($db2->Query($query)) { 
	while ($resultRow = $db2->Row() ) {
		if ($rowNum == 0) {
			siteSummaryHeader($startYear,$endYear);
		}
		$rowNum = $rowNum + 1;
		echo '<tr>';
